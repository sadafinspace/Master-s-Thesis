...
%
%    /home/space/Desktop/Sadaf/WLRT_2010-03-19_ALL.rt
%
% To extend the code to different selected data or a different text file,
% generate a function instead of a script.

% Auto-generated by MATLAB on 2017/12/08 10:29:21

%% Initialize variables.
filename = '/home/space/Desktop/Sadaf/Events data/events/WLRT_2010-03-19_ALL.rt';
startRow = 2;

%% Read columns of data as strings:
% For more information, see the TEXTSCAN documentation.
formatSpec = '%11s%9s%8s%8s%8s%10s%6s%10s%3s%5s%4s%s%[^\n\r]';
...
TIME1 = rawCellColumns(:, 2);
LON1 = cell2mat(rawNumericColumns(:, 1));
LAT1 = cell2mat(rawNumericColumns(:, 2));
ROT1 = cell2mat(rawNumericColumns(:, 3));
HEIGHT = cell2mat(rawNumericColumns(:, 4));
RATIO = cell2mat(rawNumericColumns(:, 5));
HALFANGLE = cell2mat(rawNumericColumns(:, 6));
EL = cell2mat(rawNumericColumns(:, 7));
OSRA = cell2mat(rawNumericColumns(:, 8));
NGE = cell2mat(rawNumericColumns(:, 9));
NBP = cell2mat(rawNumericColumns(:, 10));

%% Clear temporary variables
clearvars filename Date EL OSRA NGE NBP startRow formatSpec fileID dataArray ans raw col numericData rawData row regexstr result numbers invalidThousandsSeparator thousandsRegExp me rawNumericColumns rawCellColumns;

H = HEIGHT;   %Heliospheric distance in solar radius
H_smooth= smooth(H)
plot(H)
hold on
plot(H_smooth)
...
sw_velocity = 380; %c in km
y = exp(-(H_smooth-R0)./Ra);
V_sw1 = sqrt(sw_velocity^2.*(1-y));

%%  mass of CME estimation
%m0 = exp(14.7); 
m0=exp(14.7);          %true CME mass
delm =exp(14.3);      %mass increase with height 
hocc = 3.65;           %radius of occultation in Solar radius
...
plot(ACC_smooth)
 ...
 %% C_d calculation
Mcme=Mcme1(2:104,:);   %mass of cme
Acme=Acme1(2:104,:);   %making the dim same
n_H=n_H1(2:104,:);
